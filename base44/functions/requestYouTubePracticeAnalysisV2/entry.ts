...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'create_upload_ticket') {
      ...
      if (!AUDIO_EXTENSIONS.has(extension) || !Number.isFinite(audioSize) || audioSize <= 0 || audioSize > MAX_AUDIO_BYTES) {
      ...
      if (!id) return Response.json({ error: 'Guarda primero una URL válida de YouTube.' }, { status: 400 });
      ...
      const signature = await hmac(uploadSecret, `${timestamp}.${song.id}.${objectName}`);
    ...
    const workerUrl = Deno.env.get('YOUTUBE_PRACTICE_WORKER_URL');
    const requestSecret = Deno.env.get('YOUTUBE_PRACTICE_REQUEST_SECRET');
    if (!workerUrl || !requestSecret) return Response.json({ error: 'La integración de análisis aún no está configurada.' }, { status: 503 });

    const body = JSON.stringify({
      song_id: song.id,
      video_id: id,
      audio_object_name: normalizedAudioObjectName,
      catalog_chords: source.chords,
      catalog_sections: source.sections,
    });
    const timestamp = String(Date.now());
    const signature = await hmac(requestSecret, timestamp + '.' + body);
    ...
    const worker = await fetch(workerUrl.replace(/\/$/, '') + '/analyze', {
      ...
      headers: {
        ...
        'x-guitarraia-signature': signature,
      },
      body,
    });
    ...
    if (!worker.ok) {
      ...
      await base44.asServiceRole.entities.Song.update(song.id, {
        ...
        youtube_analysis_error: 'No se pudo iniciar el análisis.',
      });
