  ...
  try {
    if (name === "spotifySearch") {
      ...
      const response = await fetch(
        \`https://api.spotify.com/v1/search?type=track,artist&limit=10&q=\${encodeURIComponent(query)}\`,
    ...
    return res.status(501).json({
      error: \`Function '\${name}' is pending migration\`,
      migration_pending: true,
    });
  } catch (error) {
    res.status(500).json({ error: error.message || "Function failed" });
  }
});
`);
