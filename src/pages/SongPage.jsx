...
export default function SongPage() {
  ...
  const isPracticeMode = view === 'practicar';
  ...
  const jsonLd = song ? {
    ...
    '@graph': [
      ...
      ...(song.seo_faq?.length > 0 ? [{
        '@type': 'FAQPage',
        mainEntity: song.seo_faq.map((faq) => ({
          '@type': 'Question',
          name: faq.question,
  ...
  useEffect(() => {
    ...
    base44.entities.Song.filter({ artist_slug: artistSlug, slug: songSlug }, '-created_date', 1)
      .then((songs) => {
        ...
        setIsFav(favs.some((fav) => fav.id === found.id));
      })
      .catch(() => {})
    ...
    <div className="song-page min-h-[100dvh] w-full max-w-full lg:pb-0 bg-g-page">
      <div className="main-content mobile-page-container py-6 min-w-0" style={{ boxSizing: 'border-box' }}>
        <div className="song-header mb-5">
          <Link to={`/${artistSlug}`} className="inline-flex items-center text-sm mb-4 transition-colors" style={{ color: '#6B7280' }} onMouseEnter={(event) => { event.currentTarget.style.color = '#F97316'; }} onMouseLeave={(event) => { event.currentTarget.style.color = '#6B7280'; }}>
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0 flex items-center gap-3">
              ...
              <div className="min-w-0"><h1 className="text-2xl lg:text-3xl font-bold break-words" style={{ color: '#1F2937' }}>{displayTitle}</h1><p className="mt-1 text-base break-words" style={{ color: '#6B7280' }}>{song.artist_name}</p></div>
            </div>
            <div className="flex items-center gap-1 shrink-0">
              <button onClick={toggleFavorite} className="p-2 rounded-xl transition-colors" style={{ color: isFav ? '#F97316' : '#9CA3AF' }}><Heart className={`w-5 h-5 ${isFav ? 'fill-current' : ''}`} /></button>
              <button className="p-2 rounded-xl transition-colors" style={{ color: '#9CA3AF' }}><Share2 className="w-5 h-5" /></button>
            </div>
          </div>
        </div>
          ...
          <div className="song-reading-column min-w-0">
            ...
            {isPracticeMode ? (
              <>
                ...
                <Link to={normalLink} className="mt-5 inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-bold" style={{ borderColor: '#FDBA74', color: '#C2410C' }}>
                  <ArrowLeft className="h-4 w-4" /> Volver a acordes y Spotify
                </Link>
              </>
            ) : (
              <>
                {hasYouTubePractice(resolvedSong) && <Link to={`/${artistSlug}/${songSlug}/practicar`} className="mt-6 inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold text-white" style={{ backgroundColor: '#FF0000' }}><Youtube className="h-4 w-4" /> Practicar con IA - YouTube</Link>}
                ...
                {(hasChords || hasTablature) && <div className="flex items-center gap-1 mb-6" style={{ borderBottom: '1px solid #E5E7EB' }}>
                  {hasChords && <Link to={`/${artistSlug}/${songSlug}/acordes`} className="px-4 py-3 text-sm font-medium border-b-2 transition-colors" style={activeView === 'acordes' || isTransposed ? { borderColor: '#F97316', color: '#F97316' } : { borderColor: 'transparent', color: '#6B7280' }}>Acordes</Link>}
                  {hasTablature && <Link to={`/${artistSlug}/${songSlug}/tablatura`} className="px-4 py-3 text-sm font-medium border-b-2 transition-colors" style={activeView === 'tablatura' ? { borderColor: '#F97316', color: '#F97316' } : { borderColor: 'transparent', color: '#6B7280' }}>Tablatura</Link>}
                </div>}
                ...
                <div className="mt-8 pt-6" style={{ borderTop: '1px solid #E5E7EB' }}>
                  <div className="song-actions">
                    <button onClick={() => navigate('/chat')} className="inline-flex min-w-0 items-center justify-center gap-2 rounded-xl text-white font-semibold text-sm transition-opacity hover:opacity-90" style={{ background: 'linear-gradient(135deg, #FDBA74 0%, #F97316 100%)' }}><Sparkles className="w-4 h-4 shrink-0" /><span>Preguntar a GuitarraIA</span></button>
                    <a href="https://paypal.me/schoolmarketing/1" target="_blank" rel="noopener noreferrer" className="inline-flex min-w-0 items-center justify-center gap-2 rounded-xl font-semibold text-sm transition-opacity hover:opacity-90" style={{ backgroundColor: '#0070BA', color: '#fff' }}><span>💙 Donar U$1</span></a>
                  </div>
                </div>
              </>
            )}
