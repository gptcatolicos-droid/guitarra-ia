...
function getPracticeForSong(song) {
  ...
  return {
    ...
    sectionsEstimated: Boolean(map.sections_estimated),
  };
}
    ...
    <section className="mt-6 overflow-hidden rounded-3xl border bg-white shadow-sm" style={{ borderColor: '#FED7AA' }} aria-labelledby="youtube-practice-title">
      <div className="flex flex-col gap-3 px-5 pt-5 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm" style={{ color: '#6B7280' }}>Acordes detectados desde audio autorizado y validados contra el cifrado de esta canción.</p>
        </div>
      ...
      <div className="border-t px-5 py-6" style={{ borderColor: '#FFEDD5' }}>
        <div className="flex flex-wrap items-center justify-between gap-2">
          <p className="text-sm font-semibold" style={{ color: '#374151' }}>{practice.sectionsEstimated ? 'Sección estimada:' : 'Sección actual:'} <span style={{ color: '#EA580C' }}>{activeSection?.label || 'Cifrado'}</span></p>
