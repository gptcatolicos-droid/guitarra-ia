...
const navItems = [
  ...
  { key: 'infographics', icon: Images, label: 'Infografías', path: '/infografias' },
  { key: 'store', icon: ShoppingBag, label: 'Guitar Store', path: '/tienda', newTab: true },
];
...
export default function Sidebar({ open, onClose }) {
  ...
  const visibleItems = navItems.filter((item) => visibility[item.key] !== false);

  return (
    <>
      {open && (
        <div className="lg:hidden fixed inset-0 z-40" style={{ backgroundColor: 'rgba(15,23,42,0.45)' }} onClick={onClose} />
      )}
      ...
      <aside
        ...
        style={{ width: '240px', backgroundColor: '#FFFFFF', borderRight: '1px solid #E5E7EB' }}
      >
        <div className="flex items-center justify-end px-4 lg:hidden" style={{ height: '56px', borderBottom: '1px solid #E5E7EB', flexShrink: 0 }}>
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg" style={{ color: '#9CA3AF' }} aria-label="Cerrar menú">
            <X className="w-4 h-4" />
          </button>
        </div>
          ...
          {visibleItems.map((item) => {
            ...
            return (
              <Link
                ...
                style={active ? { backgroundColor: '#FFF1E0', color: '#C2410C' } : { color: '#6B7280' }}
              ...
              >
                {active && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 rounded-r-full" style={{ backgroundColor: '#F97316' }} />}
                <item.icon className="w-[18px] h-[18px] shrink-0" />
                <span className="flex-1">{item.label}</span>
                {item.badge && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full" style={{ backgroundColor: '#F97316', color: '#fff' }}>{item.badge}</span>}
              </Link>
            );
          })}
        </nav>
        ...
        {visibility.chat !== false && (
          <div className="p-3 pb-5" style={{ borderTop: '1px solid #E5E7EB' }}>
            <Link to="/chat" onClick={onClose} className="flex items-center gap-3 px-4 py-3.5 rounded-xl font-bold text-sm transition-opacity hover:opacity-90" style={{ background: 'linear-gradient(135deg, #FDBA74 0%, #F97316 100%)', color: '#fff', boxShadow: '0 6px 18px rgba(15,23,42,0.08)' }}>
              <MessageCircleMore className="w-5 h-5 shrink-0" />
              <span>Chat IA</span>
